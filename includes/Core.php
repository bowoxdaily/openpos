...
class Openpos_Core
{
    ...
    public function getProducts($args = array(),$display_variable = false)
    {
        ...
        if((isset($args['s']) && $args['s'] != '') || $display_variable )
        {
            ...
            {
                return $this->getSqlProducts($args,$display_variable);
            }else{
    ...
    public function getSqlProducts($args = array(),$display_variable = false){
        global $wpdb;
        $tax_args = array(
            'taxonomy'               => 'product_type',
            'orderby'                => 'name',
        ...
        $where = array();
        //start tax
        $tax_query = new WP_Tax_Query($r['tax_query']);
        ...
        {
            $meta_query = array();
            $like = $wild .$wpdb->esc_like($r['s']).$wild ;
            $meta_query[] =  $wpdb->prepare("({$wpdb->posts}.ID = %s) ",$r['s']);
            
            $meta_query[] = $wpdb->prepare("(LOWER({$wpdb->posts}.post_title) LIKE %s) ",$like);
    
            foreach($meta_args as $m)
            {
                $mlike = $wild .$wpdb->esc_like($m['value']).$wild ;
                $meta_query[] = $wpdb->prepare("( {$wpdb->postmeta}.meta_key = '%s' AND {$wpdb->postmeta}.meta_value {$m['compare']} %s ) ",$m['key'],$mlike);
            }
        ...
        {
            $where[] = "( {$wpdb->posts}.post_type = '{$args['post_type']}' )";
        }else{
            $post_type_queries = [];
            
            foreach($args['post_type'] as $post_type)
            {
                $post_type_queries[] = "{$wpdb->posts}.post_type = '{$post_type}'";
            }
            $where[] = " (".implode('  OR ',$post_type_queries).")";
        }
        if(!is_array($args['post_status']))
        {
            $where[] = " ({$wpdb->posts}.post_status = '{$args['post_status']}')";
        }else{
            $post_status_queries = [];
            foreach($args['post_status'] as $status)
            {
                $post_status_queries[] = "{$wpdb->posts}.post_status = '{$status}'";
            }
            $where[] = " (".implode('  OR ',$post_status_queries).")";
        }
        ...
        $sql .= " GROUP BY {$wpdb->posts}.ID ORDER BY {$wpdb->posts}.post_date ".strtoupper($r['order']);
        
        $page = absint( $r['current_page'] );
        if ( ! $page ) {
            $page = 1;
        }
        if ( isset( $r['offset'] ) && is_numeric( $r['offset'] ) ) {
            $r['offset'] = absint( $r['offset'] );
            $pgstrt      = $r['offset'] . ', ';
        } else {
            $pgstrt = absint( ( $page - 1 ) * $r['posts_per_page'] ) . ', ';
        }
        if($r['posts_per_page'] > 0)
        {
            $sql .= " LIMIT " . $pgstrt . $r['posts_per_page'];
        }
        ...
        $total_page = (int) ceil( $found_posts / $r['posts_per_page'] );
        return array('total'=>$found_posts,'posts' => $posts,'total_page' => $total_page);
    }
    ...
    public function getProductIdByBarcode($barcode,$accept_product_id = true){
        ...
        $use_sql = apply_filters('op_use_sql_get_product_by_barcode',true,$barcode_field,$barcode,$id);
        if($use_sql)
        {
            return $this->getProductIdByBarcodeSql($barcode_field,$barcode,array('product','product_variation'));
        }else{
            if(!$product_id)
            {
                $args = array(
    ...
    public function getProductIdByBarcodeSql($barcode_field,$barcode,$post_types = array()){
        
        global $wpdb;
        $barcode_field = sanitize_key($barcode_field); // Chỉ cho phép a-z, 0-9, _, -
        ...
        // Barcode match
        $where[] = $wpdb->prepare(" ( {$wpdb->postmeta}.meta_key = '%s' AND {$wpdb->postmeta}.meta_value = %s ) ", $barcode_field, $barcode);

        // Post type match
        if (!empty($post_types)) {
            $escaped_types = array_map('esc_sql', $post_types);
            $in_types = "'" . implode("','", $escaped_types) . "'";
            $where[] = " ( {$wpdb->posts}.post_type IN ({$in_types}) )";
        }

        $sql .= implode(" AND ", $where);
        $sql .= " ORDER BY {$wpdb->posts}.post_type DESC LIMIT 0,1";
    ...
    public function getPosRegisterOrderByDate($register_id,$from,$to,$config_status = array()){
        ...
        $final_args = apply_filters('get_pos_register_order_by_date_args',$args,$post_type,$config_status,$register_id );
    ...
    public function getPosWarehouseOrderByDate($warehouse_id,$from,$to,$config_status = array()){
        ...
        $final_args = apply_filters('get_pos_warehouse_order_by_date_args',$args,$post_type,$config_status,$warehouse_id );
        if($this->enable_hpos())
        {
            $final_args['_query_src'] = 'op_order_query';
            $data_store = WC_Data_Store::load( 'order' );
    ...
    public function getOrdersByDate($params, $config_status = array())
    {
        $date_query = array();
        ...
        if(empty($config_status))
        {
            ...
            $config_status = apply_filters('op_order_api_status',$config_status);
        }
        ...
        $final_args = apply_filters('get_api_order_by_date_args',$args,$post_type,$config_status );
       # print_r($final_args);die;
        
        if($this->enable_hpos())
        {
            $final_args['_query_src'] = 'op_order_query';
            $data_store = WC_Data_Store::load( 'order' );
            $query = $data_store->query( $final_args );
            $posts = $query->orders;
            $total_post += $query->total;
    ...
    public function getPosRegisterTransactionsByDate($register_id,$from,$to,$source_types = array(),$payment_code = ''){
        ...
        {
            $meta_query['relation'] = 'AND';
            $meta_query['_pos_transaction_cashdrawer'] = array(
                'key' => '_pos_transaction_cashdrawer',
                'value' => $register_id,
                'compare' => '=',
            );
    ...
    public function getPosWarehouseTransactionsByDate($warehouse_id,$from,$to,$source_types = array(),$payment_code = ''){
        ...
        {
            $meta_query['relation'] = 'AND';
            $meta_query['_pos_transaction_warehouse'] = array(
                'key' => '_pos_transaction_warehouse',
                'value' => $warehouse_id,
                'compare' => '=',
            );
    ...
    {
        return Carbon::parse($date.' 00:00:00', 'UTC');
    }
            ...
            case 'day':
                for($i = 1;$i<= ($diff_day +1 ); $i++)
                {
                    ...
                    $ranges[] = [
                        'from' => $tmp_start_from->toDateTimeString(),
                        'to' => $tmp_start_to->toDateTimeString(),
                        'label' => $tmp_start_from->format($date_format)
                    ];
                }
                break;
            default:
                for($i = 1;$i<= ($diff_hour +1 ); $i++)
                {
                    ...
                    $ranges[] = [
                        'from' => $tmp_start_from->toDateTimeString(),
                        'to' => $tmp_start_to->toDateTimeString(),
                        'label' => $tmp_start_from->format($time_format )
                    ];
                }
                break;
        }
    ...
    function search_customer_name($term,$limit = 5){
        ...
        $prefix = $wpdb->prefix;
        $sql = "SELECT t1.* FROM `{$wpdb->usermeta}` t1  LEFT JOIN `{$wpdb->usermeta}` t2 ON ( t1.user_id = t2.user_id AND CONCAT(t1.meta_value,' ',t2.meta_value) like '%s') WHERE ";
        ...
        if(empty($results))
        {
            $sql = "SELECT t1.* FROM `{$wpdb->usermeta}` t1  LEFT JOIN `{$wpdb->usermeta}` t2 ON ( t1.user_id = t2.user_id AND CONCAT(t1.meta_value,' ',t2.meta_value) like '%s') WHERE t1.meta_key = 'billing_first_name' AND t2.meta_key = 'billing_last_name' LIMIT 0,".$limit;
            $final_sql = $wpdb->prepare($sql ,'%'.$search_string.'%');
            $results = $wpdb->get_results($final_sql);
        }
        if(empty($results))
        {
            $sql = "SELECT t1.* FROM `{$wpdb->usermeta}` t1  LEFT JOIN `{$wpdb->usermeta}` t2 ON ( t1.user_id = t2.user_id AND CONCAT(t1.meta_value,' ',t2.meta_value) like '%s') WHERE t1.meta_key = 'shipping_first_name' AND t2.meta_key = 'shipping_last_name' LIMIT 0,".$limit;
            $final_sql = $wpdb->prepare($sql ,'%'.$search_string.'%');
            $results = $wpdb->get_results($final_sql);
        }
        $users = array();
    ...
    public function _formatCash($cash_str){
        $result = array();
        if($cash_str)
        {
